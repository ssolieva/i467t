...
def generate_binarized_contacts(traj_list, top_file, atom_indices, set1, set2, cutoff=0.4):
    ...
    for i in traj_list:
        ...
        print(i, trj)
        # use index values instead of actual residue numbers
        all_resSeqs = list(np.unique(np.concatenate((set1, set2))))
        set1_ind = [
            all_resSeqs.index(resSeq)
            for resSeq in set1
        ]
        set2_ind = []
        for r in set2: # use index values instead of actual residue numbers
            set2_ind.append(all_resSeqs.index(r))
...
def any_pairs(set1, set2, set_name, top_file, top_file_wt, path_to_traj_list, system, plot_size=(8,4)):
    ...
    atom_indices = top.top.select(' or '.join(f'residue {r}' for r in pairs_res))
    ...
    dists_bin_all = generate_binarized_contacts(traj_list, top_file, atom_indices, set1, set2) # load in trajs, get binarized contacts
